...
export interface SecondGradPdfItem {
  courseName: string;
...
type GenerateSegundaGraduacaoPdfOptions = {
  generatedAt?: Date;
};
...
export async function generateSegundaGraduacaoPdf(
  originCourseName: string,
  options: SecondGradPdfItem[],
  pdfOptions?: GenerateSegundaGraduacaoPdfOptions
) {
  const generatedAt = pdfOptions?.generatedAt ?? new Date();
  ...
  const drawHeader = () => {
    ...
    doc.setFontSize(15);
    doc.text("2ª Graduação", margin, 13);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.text(`1ª Graduação: ${originCourseName}`, margin, 20);
    doc.text(`Opções disponíveis: ${options.length}`, margin, 25.5);
  ...
  const drawColumnHeader = () => {
    ...
    const colX = {
      ...
      duration: margin + 87,
      hours: margin + 113,
      disciplines: margin + 143,
      paymentRight: pageWidth - margin - 2,
    };

    doc.text("Curso 2ª Graduação", colX.name, cursorY + 5.4);
    doc.text("Tempo", colX.duration, cursorY + 5.4);
    doc.text("Horas", colX.hours, cursorY + 5.4);
    doc.text("Discip.", colX.disciplines, cursorY + 5.4);
  ...
  options.forEach((item, index) => {
    const nameLines = doc.splitTextToSize(item.courseName, 80);
  ...
  const safeName = sanitizeFilename(originCourseName) || "graduacao";
  doc.save(`segunda-graduacao-${safeName}-${fileDate}.pdf`);
}
