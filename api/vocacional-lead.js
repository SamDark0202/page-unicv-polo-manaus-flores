...
const PROFILES = {
  ...
  Education: { name: "Educador Transformador",  emoji: "🎓", description: "Voce acredita no poder da educacao para mudar destinos. Tem paciencia para ensinar e paixao pelo conhecimento.", traits: ["Didatico", "Paciente", "Inspirador", "Humano"],            salary: "R$ 2.800 - R$ 9.000",  growth: "+28% de vagas ate 2027" },
  Creative:  { name: "Criador Inovador",         emoji: "🎨", description: "Sua mente funciona de forma unica. No mercado digital, criativos com visao estrategica sao ouro puro.", traits: ["Criativo", "Visual", "Inovador", "Expressivo"],             salary: "R$ 3.000 - R$ 14.000", growth: "+41% de vagas ate 2027" },
  ...
  Security:  { name: "Protetor Estrategico",    emoji: "🛡️", description: "Voce valoriza ordem, seguranca e protecao. Perfil para garantir a integridade de pessoas, dados e processos.", traits: ["Disciplinado", "Cauteloso", "Confiavel", "Detalhista"],    salary: "R$ 3.000 - R$ 11.000", growth: "+35% de vagas ate 2027" },
};
...
export default async function handler(request, response) {
  ...
  try {
    ...
    // PUT → envio/reenvio de e-mail de resultado via webhook Make.com
    // Caso 1 (admin): { leadId } → busca lead no banco e monta e-mail
    // Caso 2 (quiz):  { email, nome, html } → encaminha HTML já montado
    if (request.method === "PUT") {
      if (body?.leadId) {
        const leadId = String(body.leadId).trim();
        if (!isUuidLike(leadId)) return response.status(400).json({ error: "leadId inválido." });
        ...
        if (dbError || !lead) return response.status(404).json({ error: "Lead não encontrado." });
        ...
        if (!webhookRes.ok) {
          ...
          console.error("[vocacional-lead PUT/leadId] Make.com erro:", webhookRes.status, text);
      ...
      // Proxy direto do quiz (html já montado pelo frontend)
      const email = String(body?.email || "").trim();
      const nome  = String(body?.nome  || "").trim();
      const html  = body?.html;

      if (!email || !nome || !html) return response.status(400).json({ error: "Campos email, nome e html são obrigatórios." });
      if (!isEmailValid(email)) return response.status(400).json({ error: "E-mail inválido." });
